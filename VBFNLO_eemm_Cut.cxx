...
// create histograms
    InitHistVar("l1_pt, l2_pt, l3_pt, l4_pt", "all", 24, 0, 240.e3);
    InitHistVar("l1_eta, l2_eta, l3_eta, l4_eta", "all", 20, -3, 3.0);
    InitHistVar("l1_phi, l2_phi, l3_phi, l4_phi", "all", 20, -3.5,3.5);
    InitHistVar("l1_e, l2_e, l3_e, l4_e", "all", 50, 0, 500.e3);
    InitHistVar("leadingj_pt, subleadingj_pt", "all", 30, 0, 300.e3);
    InitHistVar("leadingj_eta, subleadingj_eta", "all", 20, -4.0, 4.0);
    InitHistVar("leadingj_phi, subleadingj_phi", "all", 20, -3.5, 3.5);
    InitHistVar("z1_pt, z2_pt, zz_pt", "all", 30, 0, 300.e3);
    InitHistVar("z1_eta, z2_eta, zz_eta", "all", 20, -3.0, 3.0);
    InitHistVar("z1_phi, z2_phi, zz_phi", "all", 20, -3.5, 3.5);
    InitHistVar("z1_m, z2_m", "all", 50, 40, 140.e3);
    InitHistVar("zz_m", "all", 50, 0, 500.e3);
    InitHistVar("jj_pt", "all", 30, 0, 300.e3);
    InitHistVar("jj_eta", "all", 20, -4.0, 4.0);
    InitHistVar("jj_phi", "all", 20, -3.5, 3.5);
    InitHistVar("jj_m", "all", 50, 0, 1000.e3);
    InitHistVar("delta_R_ll_1, delta_R_ll_2, delta_R_zz, delta_R_jj, delta_R_zz_jj", "all", 20, 0, 5);
    InitHistVar("mcweight", "all", 10, 0, 5);
    InitHistVar("delta_eta_jj", "all", 40, 0, 10);
    InitHistVar("centrality", "all", 40, -10, 10);
	...
	if(FLAG["none"] && l1_pt > 7 && l2_pt > 7 && l3_pt > 7 && l4_pt > 7)
	{   
	    FLAG["lepPt"] = true;   
	    m_lepPt++;
	}

	if(FLAG["lepPt"] && fabs(l1_eta) < 2.5 && fabs(l2_eta) < 2.5 && fabs(l3_eta) < 2.5 && fabs(l4_eta)< 2.5)
	{   
	    FLAG["lepEta"] = true;   
	    m_lepEta++;
	}
...
// }
	...
	if(FLAG["lepEta"] && ( l1_pt > 25 || l3_pt > 25 ) )
	{
	    FLAG["leadingLepPt"] = true;
	    m_leadingLepPt++;
	}

	if( FLAG["leadingLepPt"] && ( fabs(z1_m - Z_MASS) < 25 || fabs(z2_m - Z_MASS) < 25 ) )
	{   
	    FLAG["z1"] = true;   
	    m_z1++;
	}
	
	if( FLAG["z1"] && ( fabs(z2_m - Z_MASS) < 25 && fabs(z2_m - Z_MASS) < 25 ) )
	{   
	    FLAG["z2"] = true;   
	    m_z2++;
	}

	if( FLAG["z2"] && subleadingj_pt > 25 )
	{   
	    FLAG["numJet"] = true;   
	    m_numJet++;
	}
    ...
    cout << "none cut: " << m_none << "\n"
	 << "lepEta events: " << m_lepEta << "\n"
	 << "lepPt events: " << m_lepPt << "\n"
