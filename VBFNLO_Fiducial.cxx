...
// dealing with VBFNLO 4e or 4mu channel
...
void fiducial(string input_file, string output_file) 
{
	...
	for (int i=0; i<10; i++)
	{
	    ...
	    else if (abs(pdgID[i]) == 0 || abs(pdgID[i]) == 1 || abs(pdgID[i]) == 2 || abs(pdgID[i])==3 || abs(pdgID[i]) == 4 || pdgID[i] == 5 )
	    {
	...
	if ( index_l == 4 )
	{
		...
		if ( abs(z[1].M() - Z_MASS) < abs(z[0].M() - Z_MASS))
		{
